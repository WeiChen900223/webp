      ...
      return ( 
        <div> 
          ...
          <div> 
              ...
              <div key = {`item_${index}`}>
                  <input type='checkbox' id = {index} onClick={() => this.handleDone(index)}></input>
                  {this.state.itemDone[index] ? 
                    <label for={index} style={{textDecoration:'line-through'}}>{item}</label>: <label for={index}>{item}</label>}
